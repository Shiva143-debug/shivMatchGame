...
// let active = Math.ceil(Math.random() * (imagesList.length - 1))
...
class AppStore extends Component {
  state = {
    ...
    activeId: Math.ceil(Math.random() * (imagesList.length - 1)),
  }
  ...
  playAGAIN = () => {
    this.setState({time: 60, timeup: false})
  }
  ...
  ButtonClick = id => {
    ...
    if (id === activeId) {
      ...
      this.setState(prevState => ({
        ...
        activeId: Math.floor(Math.random(1) * 30),
      }))
    } else {
      this.setState({
        activeId: null,
  ...
  //     return <img className="main-image" alt="match" src={imageUrl} />
  //   }
  ...
  headerFun = () => {
    ...
    return (
      <nav className="Nav">
        <ul className="time-text-container">
  ...
  render() {
    const {activeTabId, score, timeup, activeId} = this.state
    const {imageUrl} = imagesList[activeId]
    ...
    return (
      <div className="App-container">
        ...
        {!timeup && (
          <div>
            <img className="main-image" alt="match" src={imageUrl} />
        ...
        {/* {timeup && <h1 className="good">Head</h1>} */}
        {timeup && <GameOver score={score} playAGAIN={this.playAGAIN} />}
        {/* <GameOver countDetails={this.count} /> */}
      </div>
    )
  }
}
